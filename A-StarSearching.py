...
def create_cameFrom(self):
    ...
    #Camefrom=dict.fromkeys(keys,None)
    Camefrom={}
    return Camefrom
...
def create_gScore(self):
    ...
    # be set to infinity.
    keys=list(range(0,40));
    gScore=dict.fromkeys(keys,float('Inf'))
    gScore[self.start]=0
    return gScore
...
def create_fScore(self):
    ...
    # set to infinity.
    keys=list(range(0,40));
    fScore=dict.fromkeys(keys,float('Inf'))
...
def get_current_node(self):
    ...
    for node in self.openSet:
        if(temp>self.fScore[node]):
            finalnode=node
            temp=self.fScore[node]
    return finalnode
...
def distance(self, node_1, node_2):
    ...
    x2=self.map.intersections[node_1][0]
    x1=self.map.intersections[node_2][0]
    y2=self.map.intersections[node_1][1]
    y1=self.map.intersections[node_2][1]
...
def record_best_path_to(self, current, neighbor):
    ...
    self.fScore[neighbor]=(self.gScore[neighbor]+self.distance(neighbor,self.goal))
...
# 
# **ANSWER**:
# A-star is like acheiving long term goal while keeping track of short term spendings and keep them limited to make sure not to overspend.

# ---
# How does A-Star search algorithm differ from Uniform cost search? What about Best First search?
# 
# **ANSWER**:
# In Overall A - Star appears like mix of Uniform search which tries to expand via least cost but does not keep track pf number of nodes visited. Whereas Best first try to visit less number of nodes but tend to compromise with total path cost in some cases.

# ---
# What is a heuristic?
# 
# **ANSWER**: Heuristic can be seen as primary estimate of goal, which gradually converges to zero as we reach to goal.

# ---
# What is a consistent heuristic?
# 
# **ANSWER**:After finding path , if total path cost is greater than or equal to heuristic cost, then is is suposed to be consistent with what assuption we made at start.

# ---
# What is a admissible heuristic? 
# 
# **ANSWER**:The heuristic function shall not return a vlue higher than possible short path, then that is called as admissible heuristic, which never overestimate.
# 
